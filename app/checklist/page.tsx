...
export default
cat > app/checklist/page.tsx << 'ENDOFFILE'
"use client";
      ...
      <div className="mx-auto max-w-5xl px-6 py-12">
        ...
        {showGenerator && (
          <div className="rounded-[32px] bg-[#f8f7ff] p-8 mb-12">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {NATIONALITY_TYPES.map((t) => (
                    <button key={t.value} onClick={() => setNationalityType(t.value)} className={`w-full rounded-2xl px-4 py-3 text-sm font-bold text-left transition ${nationalityType === t.value ? "bg-violet-600 text-white" : "bg-white border-2 border-gray-200 text-gray-600 hover:border-violet-400"}`}>
                      {t.label}
                    </button>
                  ))}
                </div>
            ...
            {checklists.length > 0 && (
              <button onClick={() => setShowGenerator(false)} className="mt-3 w-full rounded-2xl border-2 border-gray-200 px-6 py-3 text-sm font-bold text-gray-500 transition hover:border-violet-400">
                Cancel
              </button>
            )}
          </div>
        ...
        {activeChecklist && !showGenerator && !generating && (
          <div>
            <div className="flex items-start justify-between mb-8">
              ...
              <button onClick={() => handleDelete(activeChecklist.id)} className="rounded-2xl border-2 border-red-200 px-4 py-2 text-sm font-bold text-red-500 transition hover:bg-red-50">
                🗑️ Delete
              </button>
            </div>
            ...
            <div className="mt-12 rounded-[32px] bg-[#f8f7ff] p-8 text-center">
              ...
              <button onClick={() => setShowGenerator(true)} className="inline-block rounded-2xl bg-gradient-to-r from-violet-600 via-pink-500 to-orange-400 px-8 py-4 text-sm font-bold text-white shadow-2xl transition hover:scale-105">
...
ENDOFFILEcat > app/checklist/page.tsx << 'ENDOFFILE'
"use client";
