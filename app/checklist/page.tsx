...
export default function ChecklistPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/checklist", {
        ...
        body: JSON.stringify({ userId: user.id, city, country, nationality, nationalityType, employmentType, familySituation }),
      });
  ...
  const handleToggleItem = async (sectionIndex: number, itemIndex: number) => {
    ...
    updated.items = updated.items.map((section, si) => ({
      ...
      items: section.items.map((item, ii) => {
        if (si === sectionIndex && ii === itemIndex) return { ...item, completed: !item.completed };
        return item;
      }),
    }));
  ...
  const handleDelete = async (checklistId: string) => {
    ...
    if (remaining.length > 0) { setActiveChecklist(remaining[0]); }
    else { setActiveChecklist(null); setShowGenerator(true); }
  };
  ...
  if (!user) return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center text-[#0B1957]">
      <div className="text-center max-w-md px-6">
        ...
        <Link href="/auth/login" className="inline-block rounded-2xl bg-gradient-to-r from-violet-600 via-pink-500 to-orange-400 px-8 py-4 text-sm font-bold text-white shadow-2xl transition hover:scale-105">Sign in →</Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-white text-[#0B1957]">
      ...
      <section className="bg-gradient-to-br from-[#0B1957] via-violet-900 to-pink-900 px-6 py-16 text-white text-center">
        ...
        <h1 className="text-5xl font-black leading-none md:text-6xl mb-4">Stay on track 📋</h1>
        <p className="mx-auto max-w-2xl text-lg text-white/80">Your personalised relocation checklist — tailored to your nationality, destination and situation.</p>
      </section>
      ...
      <div className="mx-auto max-w-5xl px-6 py-12">
        ...
        {checklists.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowGenerator(true)} className="rounded-2xl bg-gradient-to-r from-violet-600 via-pink-500 to-orange-400 px-4 py-2 text-sm font-bold text-white transition hover:scale-105">+ New checklist</button>
            </div>
            <div className="flex flex-wrap gap-3">
              {checklists.map((c) => {
                const prog = getProgress(c);
                return (
                  <button key={c.id} onClick={() => { setActiveChecklist(c); setShowGenerator(false); }} className={`rounded-2xl px-4 py-3 text-sm font-bold transition text-left ${activeChecklist?.id === c.id ? "bg-violet-600 text-white" : "bg-[#f8f7ff] text-[#0B1957] hover:bg-violet-100"}`}>
                    <div>{c.city}, {c.country}</div>
                    <div className={`text-xs mt-1 ${activeChecklist?.id === c.id ? "text-white/70" : "text-gray-400"}`}>{prog.completed}/{prog.total} completed</div>
                  </button>
                );
              })}
            </div>
        ...
        {showGenerator && (
          <div className="rounded-[32px] bg-[#f8f7ff] p-8 mb-12">
            ...
            {checklists.length > 0 && (
              <button onClick={() => setShowGenerator(false)} className="mt-3 w-full rounded-2xl border-2 border-gray-200 px-6 py-3 text-sm font-bold text-gray-500 transition hover:border-violet-400">Cancel</button>
            )}
          </div>
        )}
        ...
        {generating && (
          <div className="text-center py-20">
            ...
            <div className="mt-8 flex justify-center gap-2">
              {[1,2,3,4].map((i) => <div key={i} className="w-2 h-2 rounded-full bg-violet-400 animate-pulse" style={{ animationDelay: `${i * 0.2}s` }} />)}
            </div>
          </div>
        )}
          ...
          <div>
            <div className="flex items-start justify-between mb-8">
              <div>
                ...
                <p className="text-gray-500 mt-1">{activeChecklist.city}, {activeChecklist.country} • {activeChecklist.nationality}</p>
              </div>
              <button onClick={() => handleDelete(activeChecklist.id)} className="rounded-2xl border-2 border-red-200 px-4 py-2 text-sm font-bold text-red-500 transition hover:bg-red-50">🗑️ Delete</button>
            </div>
              ...
              return (
                <div className="rounded-[24px] bg-[#f8f7ff] p-6 mb-8">
                  ...
                  <div className="h-4 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-violet-600 via-pink-500 to-orange-400 rounded-full transition-all duration-500" style={{ width: `${prog.percent}%` }} />
                  </div>
            ...
            <div className="space-y-8">
              {activeChecklist.items.map((section, sectionIndex) => (
                <div key={section.title}>
                  <h3 className="text-xl font-black mb-4">{section.emoji} {section.title}</h3>
                  <div className="space-y-3">
                    {section.items.map((item, itemIndex) => (
                      <div key={item.id} className={`rounded-[20px] border-2 p-5 transition ${item.completed ? "border-green-200 bg-green-50" : "border-gray-100 bg-white"}`}>
                        <div className="flex gap-4 items-start">
                          <button onClick={() => handleToggleItem(sectionIndex, itemIndex)} className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 transition ${item.completed ? "bg-green-500 border-green-500 text-white" : "border-gray-300 hover:border-violet-500"}`}>
                            {item.completed && <span className="text-xs">✓</span>}
                          </button>
                          <div className="flex-1">
                            <p className={`font-bold text-sm ${item.completed ? "line-through text-gray-400" : ""}`}>{item.task}</p>
                            {!item.completed && (
                              <>
                                  ...
                                  <div className="mt-3">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {item.documents.map((doc) => (
                                        <span key={doc} className="rounded-full bg-violet-100 px-3 py-1 text-xs font-semibold text-violet-700">{doc}</span>
                                      ))}
                                    </div>
                                  </div>
                                )}
            ...
            <div className="mt-12 rounded-[32px] bg-[#f8f7ff] p-8 text-center">
              ...
              <button onClick={() => setShowGenerator(true)} className="inline-block rounded-2xl bg-gradient-to-r from-violet-600 via-pink-500 to-orange-400 px-8 py-4 text-sm font-bold text-white shadow-2xl transition hover:scale-105">+ Create new checklist</button>
            </div>
          </div>
        )}
      </div>
